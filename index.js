// ===== index.js — My Café (MySQL Edition) =====
...
// ===== ข้อมูลหมวดหมู่จาก DB =====
let categoriesData = []; // [{ category_id, name, emoji }]
let allMenuItems = []; // flat list ของทุกเมนูจาก DB

// ===== โหลดเมนูจาก MySQL API หรือ fallback default =====
async function loadMenuData() {
    try {
        // ดึง categories + menu พร้อมกัน
        const [catRes, menuRes] = await Promise.all([
            fetch('/codetest/api/categories.php'),
            fetch('/codetest/api/menu.php'),
        ]);

        if (!catRes.ok || !menuRes.ok) throw new Error('API response not OK');
        ...
        if (Array.isArray(categories) && categories.length &&
            ...
            // สร้าง menu tabs จาก categories ที่ดึงมา
            renderMenuTabs(categories);

            // สร้าง Recommended slider จากเมนูที่ is_featured = 1
            renderRecommended(menuItems);

            return grouped;
        }
    } catch (e) {
        console.warn('MySQL API menu load failed, using default data', e);
    }

    // fallback — ใช้ข้อมูลเดิม + render tabs แบบ default
    renderMenuTabsDefault();
    renderRecommendedDefault();
    return defaultMenuData;
}

// ===== RENDER MENU TABS จาก DB =====
function renderMenuTabs(categories) {
    const container = document.getElementById('menuTabs');
    if (!container) return;
    container.innerHTML = categories.map((cat, idx) => {
        const emoji = cat.emoji || '📁';
        return `<div class="menu-tab${idx === 0 ? ' active' : ''}" onclick="switchCategory('${cat.category_id}', this)"><h3>${emoji} ${cat.name}</h3></div>`;
    }).join('');
}

// ===== RENDER MENU TABS fallback (ถ้า API ไม่ได้) =====
function renderMenuTabsDefault() {
    const container = document.getElementById('menuTabs');
    if (!container) return;
    const defaults = [
        { key: 'food', emoji: '🍜', name: 'เมนูอาหาร' },
        { key: 'snack', emoji: '🍤', name: 'เมนูขนม' },
        { key: 'dessert', emoji: '🍰', name: 'เมนูของหวาน' },
        { key: 'drink', emoji: '🧋', name: 'เมนูเครื่องดื่ม' },
    ];
    container.innerHTML = defaults.map((d, idx) =>
        `<div class="menu-tab${idx === 0 ? ' active' : ''}" onclick="switchCategory('${d.key}', this)"><h3>${d.emoji} ${d.name}</h3></div>`
    ).join('');
}

// ===== RENDER RECOMMENDED (is_featured) จาก DB =====
function renderRecommended(menuItems) {
    const track = document.getElementById('topTrack');
    if (!track) return;
    const featured = menuItems.filter(i => i.is_featured && i.is_available);
    if (featured.length === 0) {
        // ถ้าไม่มี featured เลย ใช้ default
        renderRecommendedDefault();
        return;
    }
    track.innerHTML = featured.map(item => `
        <div class="product">
            ...
            <button class="add-to-cart" onclick="openModal('${escStr(item.emoji || '🍽️')}','${escStr(item.name)}','${escStr(item.description || '')}',${item.price})">🛒 เพิ่มลงตะกร้า</button>
        </div>
    `).join('');
}

// ===== RENDER RECOMMENDED fallback =====
function renderRecommendedDefault() {
    const track = document.getElementById('topTrack');
    if (!track) return;
    const items = defaultMenuData.food;
...
function searchMenu() {
    ...
    if (query) {
        ...
        tabs.forEach(t => {
            if (t.getAttribute('onclick')?.includes("'" + currentCategory + "'")) {
...
(async () => {
    ...
    // ใช้ category แรกจาก DB (หรือ 'food' ถ้าใช้ default)
    const firstCat = Object.keys(menuData)[0] || 'food';
    renderCards(firstCat);
    updateCart();
    checkExistingOrders();
})();